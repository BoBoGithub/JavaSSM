            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
           		 <div class="uc m-l-5 m-r-5" role="form" style="padding:10px 0px;">
	           		 <div class="form-horizontal" role="form">
	           		 		 <div class="form-group">
								    <label for="orderId" class="col-sm-2 control-label">借款单ID：</label>
								    <div class="col-sm-1">
								      <input type="text" class="form-control" style="width: 150px;;" id="orderId" placeholder="请输入借款单ID">
								    </div>
								    ...
								    <label for="mobile" class="col-sm-2 control-label">用户电话：</label>
								    <div class="col-sm-1">
			           		   ...
			           		   <div class="form-group">
								   ...
								   <div class="col-sm-2 btn-group bootstrap-select show-tick">
								    ...
								    <label for="orderStatus" class="col-sm-2 control-label" style="margin-left:-85px;">订单状态：</label>
									<div class="col-sm-2 btn-group bootstrap-select show-tick">
											...
											<ul class="order-status-class dropdown-menu open inner" role="menu" style="max-height: 130px; overflow-y: auto;cursor:pointer;">
												<li data-original-index="" class="selected active"><a>全部</a></li>
												<li data-original-index="-1"><a>审核失败</a></li>
												<li data-original-index="0"><a>待审核</a></li>
												<li data-original-index="1"><a>待人工确认</a></li>
												<li data-original-index="2"><a>待确认银行卡</a></li>
												<li data-original-index="3"><a>待签约</a></li>
												<li data-original-index="4"><a>待放款</a></li>
												<li data-original-index="5"><a>放款失败</a></li>
												<li data-original-index="6"><a>待还款</a></li>
												<li data-original-index="7"><a>还款中</a></li>
												<li data-original-index="8"><a>已还款</a></li>
												<li data-original-index="9"><a>逾期</a></li>
												<li data-original-index="10"><a>坏账</a></li>
											</ul>
										</div>  
							  </div>
							 ...
							 <div class="form-group">
								    <label for="checkType" class="col-sm-2 control-label">审核类型：</label>
								      <div class="radio">
											<label>
												<input type="radio" name="checkType" value="" checked>全部
											</label>
											<label>
												<input type="radio" name="checkType" value="0" > 自动
											</label>
											<label>
												<input type="radio" name="checkType"  value="1" > 人工
											</label>
											<label>
												<input type="radio" name="checkType"  value="2" > 百融
											</label>
										</div>
								</div>
					...
					<div id="user_list_table_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
						<table class="table table-striped table-bordered table-hover  dataTable no-footer" id="user_list_table" role="grid" aria-describedby="user_list_table_info" style="width: 1025px;">
						    <thead>
						        <tr role="row">
						            ...
						            <th class="text-center sorting"  style="width: 86px;">手机号</th>
						            <th class="text-center sorting"  style="width: 116px;">借款状态</th>
						            <th class="text-center sorting"  style="width: 118px;">风控评分</th>
						            <th class="text-center sorting"  style="width: 86px;">审核人</th>
						            <th class="text-center sorting"  style="width: 116px;">申请时间</th>
						            <th class="text-center sorting"  style="width: 116px;">操作</th>
						        </tr>
						    </thead>
...
<script>
		...
		if(data.length > 0){
			$.each(data, function(item){
				...
				html += "<tr role='row'>"+
										...
										"<td>"+data[item].operater+"</td>"+
										"<td>"+data[item].applyTime+"</td>"+
										"<td> <a href='javascript:void(0);' onclick=\"Common.loadPage('订单管理', '借款订单管理', '订单明细', '${basePath}/admin/loan/order/detail?orderId="+data[item].brecordId+"', true)\">订单明细</a> <a href='javascript:void(0);' onclick='showOrderStatus("+data[item].brecordId+");'>状态明细</a> </td>"+
								"</tr>";
			});
		}else{
	...
	function orderDetail(orderId){
		//加载页面
		Common.loadPage('订单管理', '借款订单管理', '订单明细', '${basePath}/admin/loan/order/detail/'+orderId, true);
	}
	...
	function showOrderStatus(orderId){
		...
		layer.open({
			  ...
			  type: 2,
			  area: ['720px', '407px'],
			  content: ['${basePath}/admin/loan/order/status/list?orderId='+orderId, 'no'],
		});
	}
