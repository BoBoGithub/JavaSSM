	           		 		 ...
	           		 		 <div class="form-group">
			           		   ...
			           		   <div class="form-group">
									...
									<div class="col-sm-2 btn-group bootstrap-select show-tick">
											...
											<button type="button"  class="btn dropdown-toggle btn-default" data-toggle="dropdown" data-id="first-disabled2"  aria-expanded="false">
												<span class="filter-option pull-left" id="orderStatusText">待审核</span>&nbsp;<span class="caret"></span>
											</button>
											<ul class="order-status-class dropdown-menu open inner" role="menu" style="max-height: 130px; overflow-y: auto;cursor:pointer;">
												<li data-original-index=""><a>全部</a></li>
												<!-- 
													待审核：机器审核通过后　人工还未审核
													已审核：机器审核通过后　人工也已审核
												 -->
												<li data-original-index="1" class="selected active"><a>待审核</a></li>
												<li data-original-index="2"><a>已审核</a></li>
												<li data-original-index="-2"><a>审核失败</a></li>
											</ul>
									</div> 
										
								    <label for="operater" class="col-sm-2 control-label" style="margin-left:-85px;">操作人：</label>
								    <div class="col-sm-1">
								      <input type="text" class="form-control" style="width: 150px;" id="operater" placeholder="请输入操作人">
								    </div>
							  </div>
						...
						<table class="table table-striped table-bordered table-hover  dataTable no-footer" id="user_list_table" role="grid" aria-describedby="user_list_table_info" style="width: 97%;">
						    <thead>
						        <tr role="row">
						            ...
						            <th class="text-center sorting"  style="width: 126px;">操作</th>
						        </tr>
						    </thead>
...
<script>
	...
	function getListData(pageNum){
		...
		//检查风控最低分数
		if(scoreStart != ''){
			if(!isNaN(scoreStart) && (scoreStart < 10 || scoreStart >= 50)){
				//选中最小分数
				$("#scoreStart").select();
				layer.msg("最小数据评分不能小于10 不能大于50");
				return false;
			}
		}else{
			scoreStart = 10;
		...
		//检查风控最高分数
		if(scoreEnd != ''){
			if(!isNaN(scoreEnd) && (scoreEnd >= 50 || scoreEnd < 10)){
				//选中最小分数
				$("#scoreEnd").select();
				layer.msg("最大数据评分不能小于10 不能大于50");
				return false;
			}
		}else{
			scoreEnd = 50;
		...
		if(data.length > 0){
			$.each(data, function(item){
				...
				html += "<tr role='row'>"+
						...
						if(data[item].status == "待人工信审"){
							html +=	"<td> <a href='javascript:void(0);' onclick=\"Common.loadPage('订单管理', '借款单管理', '审核借款单', '${basePath}/admin/loan/order/check?orderId="+data[item].brecordId+"')\">审核</a> <a href='javascript:void(0);' onclick='refuseOrder("+data[item].brecordId+", true);'>拒绝</a> 风险报告</td>";	
						}else{
							html +=	"<td> <span style='color:gray;'>审核</span> <span style='color:gray;'>拒绝</span> 风险报告</td>";							
						}							
					html += "</tr>";
			});
		}else{
		...
		$.ajax({
            ...
            success: function (json) {
          	  if(json.errno == 0 && json.data.status){
	          	  ...
	          	  //跳转　返回员页面
	          	  setTimeout(function(){Common.loadPage('订单管理', '借款单管理', '审核借款单', '${basePath}/admin/loan/check/list', true)}, 2000);
          	  }else{
          		  //提示信息
          		  layer.msg(json.errmsg);
