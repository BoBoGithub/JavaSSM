 ...
 */
@Component("rabbitＭqFanoutService")
public class FanoutReceiver implements MessageListener {
	//引入日志操作类
	...
	public void onMessage(Message message) {
		//记录接收队列参数
		//logger.debug("FanoutQueue消息消费者＝{}", message.toString());
		...
		try {
			...
			//字符串转换成JSON串
			logger.debug("FanoutQueue消息消费者接收JSON：{}", jsonObject);
			
			//处理消息内容
			queueService.dealQueueTask(jsonObject);
		} catch (Exception e) {
			//处理失败记录日志
			logger.debug("FanoutQueueReceiver onMessage exception: "+e.getMessage());
		}
	}
}
